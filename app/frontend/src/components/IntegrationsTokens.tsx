...
import { ArrowLeft, Upload, Download, Plus, Eye, EyeOff, Check, X, AlertCircle } from 'lucide-react';
...
export const IntegrationsTokens: React.FC<IntegrationsTokensProps> = ({ language, onNavigate }) => {
  ...
  const handleActivate = (rowIndex: number) => {
    ...
    const project = projects.find(p => p.name === row.project_link);
  ...
  const handleExportCSV = () => {
    const headers = [
      ...
      'tiktok_access_token', 'activate', 'activation_date', 'activation_countdown',
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1800px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginLeft: 'auto' }}>
            ...
            <button
              ...
              style={{
                ...
                gap: '8px',
                padding: '12px 20px',
                background: selectedProject ? 'linear-gradient(135deg, var(--brand-cyan), var(--brand-teal))' : 'var(--bg-secondary)',
                border: 'none',
                borderRadius: '8px',
      ...
      {!selectedProject && (
        <div className="glass-card" style={{ padding: '48px', borderRadius: '16px', textAlign: 'center' }}>
          <AlertCircle size={48} style={{ color: 'var(--accent-yellow)', margin: '0 auto 16px' }} />
          <p style={{ color: 'var(--text-secondary)', fontSize: '16px' }}>
            {language === 'EN'
              ? 'Please select a project to view and manage integrations'
              : 'Por favor selecciona un proyecto para ver y gestionar integraciones'}
          </p>
        </div>
      )}
          ...
          <div className="glass-card" style={{ padding: '24px', borderRadius: '16px', marginBottom: '24px', overflowX: 'auto' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: '13px' }}>
                ...
                <tbody>
                  {rows.filter(row => row.project_link === selectedProject || !row.project_link).map((row, idx) => (
