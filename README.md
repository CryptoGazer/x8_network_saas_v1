...
*размещён на AWS; конкретно под нужды n8n выделены 3 контейнера: n8n-n8n-1, n8n-postgres-1, n8n-traeffik-1*
...
## Supabase
*Облачное хранилище, не хостится нами*
...
WAHA — self-hosted сервис *(в частности, хоститься на AWS: поднимается в отдельном Docker-контейнере)* для подключения WhatsApp клиента через механику WhatsApp Web:
